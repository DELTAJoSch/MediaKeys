...
namespace MediaKeys
{
    static class KeyboardInteraction
    {
        [DllImport("user32.dll", CharSet = CharSet.Unicode, CallingConvention = CallingConvention.StdCall)]
        private static extern uint SendInput(uint cInputs, INPUT[] inputs, int cbSize);

        /// <summary>
        /// This function sends a keyDown and a keyUp message to the system
        /// </summary>
        /// <param name="key">the key to be pressed</param>
        internal static void SendKey(KeyCode key)
        {
            // pressed
            ...
            // released
            INPUT inputRelease = new INPUT { Type = 1 };
            inputRelease.Data.Keyboard = new KEYBDINPUT
            {
                Vk = (ushort)key,
                Time = 0,
                ExtraInfo = IntPtr.Zero,
            ...
            // array, ordered the way the presses should be executed
            INPUT[] inputs = new INPUT[] { input, inputRelease };

            //throws an exception if the keypress could not be sent, e.g. when there are too many keys being pressed at once
